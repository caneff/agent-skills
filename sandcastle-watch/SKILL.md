...
3. Write a one-line digest to `.sandcastle/logs/watch-status` in the run's repo (e.g. `🏰 iter 2/5 · 3 PRs · 1✗`) — **every tick, even when nothing changed.** The scoped `sandcastle-segment.sh` ccstatusline segment shows this file only in the window whose cwd is that repo, and drops it once it's older than 180s, so re-writing each ~90s tick is what keeps the status-bar line alive.

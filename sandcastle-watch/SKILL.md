...
`start` launches the run with **`SANDCASTLE_PR_GROUPING=spec`** by default: a whole spec lands as **one reviewable PR** instead of one PR per ticket. This is a watch default only — an explicit `SANDCASTLE_PR_GROUPING` in the environment still wins, so a user who wants one PR per issue runs `SANDCASTLE_PR_GROUPING=issue` before the skill. Expect the `→ PR #N` milestones to read one-per-spec, not one-per-issue.

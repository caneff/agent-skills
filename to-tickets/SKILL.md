...
- **A real issue tracker (GitHub, Linear, …)** → publish one issue per ticket in dependency order (blockers first) so each ticket's blocking edges can reference real identifiers. Use the platform's native blocking / sub-issue relationship where it has one; otherwise set each ticket's "Blocked by" to the blocking issues. When the source is a parent issue, link every published ticket as a child using the adapter's parent-linking convention (`docs/agents/issue-tracker.md`) — then verify the parent reports the full ticket count as children before finishing. Apply the `ready-for-agent` triage label unless instructed otherwise — the tickets are agent-grabbable by construction.

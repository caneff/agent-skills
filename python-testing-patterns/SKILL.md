...
@pytest.fixture
def connect_orders():
    """Provide a helper that builds a dataset from local test rows."""
    return build_orders_connection

def test_revenue_by_region(connect_orders):
    rows = (
...
1. **Not using fixtures**: Repeating setup code across tests
   - Solution: Create fixtures in conftest.py for setup helpers, builders, clients, schemas, and cleanup. Keep assertion-driving rows or records local to the test.

...
@pytest.fixture
def make_user():
    """Factory for user records; tests supply assertion-driving values."""
...
Keep rows, records, and domain examples in the test when expected outputs depend
on those values. Use fixtures for mechanics: empty resources, schemas, clients,
connections, cleanup, and builder functions.

**Good: local data plus shared helper**
```python
def test_revenue_is_grouped_by_region(connect_orders):
    rows = (
    ...
    with connect_orders(rows) as connection:
        result = prepare_data(connection)

    assert result == {"North": 1500.00}
```

**Bad: hidden assertion-driving data**
```python
def test_revenue_is_grouped_by_region(prepared_data):
    assert prepared_data == {"North": 1500.00}
```

Avoid fixtures such as `sample_orders`, `commerce_connection`, or
`prepared_data` when the hidden values are needed to understand the expected
result.

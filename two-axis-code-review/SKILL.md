...
**A finding names the file and the intent, not the edit.** Say where the problem is and what outcome is wrong; the fixer owns the file and picks the change. A finding written as a patch to apply verbatim ("exactly these, nothing else") turns one round into four.

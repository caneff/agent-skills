...
When a separate consumer reads the disclosed file directly — a reviewer, a hook, a script that opens it by name — disclose by moving the always-on _import_, not the file: leave the file at its path, drop only the pointer that force-loaded it. Content that consumer must see then lives in two places at once — inline in the always-on core and in the full file — and that duplication is correct, not a smell, because the copies serve different load paths.

...
# bound its retry, block "PR up" rather than report-and-hope on a still-
# missing ticket (Codex pass on PR #833: the earlier "say so" fallback let
# a PR that closes nothing reach the merge), § The merge step 2 must refuse
# to merge a PR that closes nothing, and the PR body itself must carry a
# bare `Closes #<n>` line — not just the final commit body. This is a prose
...
# fix-and-recheck blocks "PR up" outright (Codex pass on PR #833: the
# earlier "say so and send anyway" fallback let a PR that closes nothing
# reach the merge) rather than being reported and sent regardless.
check_in "$before_pr_section" 'fix the body'
check_in "$before_pr_section" 're-run this check once'
check_in "$before_pr_section" 'do not send "PR up"'
check_in "$before_pr_section" 'a PR that closes nothing must not reach the merge'
...
# Rule 5 (Codex pass on PR #833): the controller's own pre-merge gate
# refuses a PR that closes nothing, not just the worker's pre-report gate —

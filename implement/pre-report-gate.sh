...
# the PR-up report by setting PRE_REPORT_KEEP_SCRATCH="<why>", which turns
# this check into a warning instead of a failure.
...
if [ -d "$scratch_dir" ] && [ -n "$(ls -A "$scratch_dir" 2>/dev/null)" ]; then
  if [ -n "${PRE_REPORT_KEEP_SCRATCH:-}" ]; then
    echo "pre-report gate: WARNING — .scratch/ still has content, acknowledged as kept: $PRE_REPORT_KEEP_SCRATCH — quote this in the PR-up report" >&2
  else
...
echo "pre-report gate: clean tree, .scratch/ clear, ${sha:0:12} is an ancestor of ${tip} (${tip_sha:0:12})"

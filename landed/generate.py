...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "all-audits", "harness"))
import pagelib  # noqa: E402

# Dracula Theme Soft — the user's VS Code theme. Landed is dark-only by
...
.commit summary { cursor:pointer; padding:.7rem .9rem; list-style:none;
  display:grid; grid-template-columns:minmax(0,45%) minmax(0,1fr); gap:.3rem 1rem; align-items:start; }
...
def main(argv=None):
    ...
    if not tabs:
        ...
        return 1
    body = f'''<p class="range">range: {esc(range_arg)} · generated {datetime.datetime.now():%Y-%m-%d %H:%M} ·
...
{"".join(panes)}'''
    ...
    pagelib.copy_assets(out_path.parent)

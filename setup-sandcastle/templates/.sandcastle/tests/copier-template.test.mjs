...
const ARC_REWRITTEN_PROSE = {
  ...
  ".sandcastle/main.mts": [
    ...
    // #109: decideInReviewAction returns the whole plan; the sweep loop applies
    // it and decides nothing. The by-hand completed record and the branch delete
    // now come out of the function.
    [
      ...
      "    const branch = issueBranch(id);\n    const branchExists = branchExistsWithWork(id);\n    const mergesClean = branchExists && mergesCleanIntoMain(branch);\n    // Rebuild the dependency graph from GitHub blockedBy edges (issue #50);\n    // prComponents drops any parent not also completed this run, so a stacked\n    // recovery collapses into one PR instead of one per tip.\n    const parents = parentsFromBlockedBy(\n      blockedByForIssues.get(issue.number) ?? []\n    );\n    const plan = decideInReviewAction(classification, {\n      id,\n      title: issue.title,\n      branch,\n      parents,\n      branchExists,\n      mergesClean,\n    });\n\n    // Apply the plan; decideInReviewAction made every call, this loop only acts.\n    console.log(`  ${plan.note}`);\n    if (plan.deleteBranch) git(`branch -D ${branch}`); // no-op (null) if absent\n    if (plan.addLabel) relabel(id, plan.addLabel, plan.removeLabels);\n    if (plan.completed) completedFromSweep.push(plan.completed);\n    if (plan.bucket === \"injected\") sweepInjected.add(id);\n    if (plan.bucket === \"requeued\") sweepRequeued.add(id);\n  }",
    ],
  ...
  ".sandcastle/reconcile.mts": [
    ...
    // #109 — decideInReviewAction returns a full plan object (label move, branch
    // delete, summary bucket, inject record) at the depth of planGateOutcome /
    // planOutcomeTransition, so the sweep caller applies it and decides nothing.
    [
